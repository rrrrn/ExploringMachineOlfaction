...
from torchmetrics.functional import explained_variance, mean_squared_error, pearson_corrcoef, cosine_similarity
...
def train(model, loader, criterion):   
    ...
    return loss, binary_accuracy(pred,y_truth).numpy(force=True), binary_auroc( out_proba[:,1], y_truth).numpy(force=True)
...
def train_regr(model, loader, criterion):
    ...
    error["explained_variance"] = explained_variance(out_all, y_truth, multioutput = "raw_values").numpy(force=True)
    error["explained_variance_weighted"] = explained_variance(out_all, y_truth, multioutput = "variance_weighted").numpy(force=True)
    ...
    with torch.no_grad():
        for i, data in enumerate(loader,0):  # Iterate in batches over the training/test dataset.
            ...
            if len(y_truth)==0:
                ...
                out_proba = torch.vstack([out_proba,out])
        _, pred = torch.max(out_proba, 1)
    return  binary_accuracy(pred,y_truth).item(), binary_auroc(out_proba[:,1], y_truth).item(), criterion(out_proba[:,1].float(), y_truth.float()).item()
